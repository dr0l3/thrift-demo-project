...
lazy val commonDeps = Seq(
  libraryDependencies ++= Seq(
    ...
    "io.catbird" %% "catbird-finagle" % finagleVersion,
...
lazy val commonDocker = dockerfile in docker := {
  ...
  val classpath = (managedClasspath in (Compile, packageBin)).value
...
def dockerImageNames(name: String) = {
  imageNames in docker := Seq(
    // Sets the latest tag
    ImageName(s"dr0l3/demo-$name:latest")
  )
}
...
lazy val protocols = baseproject("protocols")
  .settings(
    (scroogeThriftOutputFolder in Compile) := {(sourceManaged in Compile) (_ / "scrooge")}.value,
    (scroogeThriftOutputFolder in Test) := {(sourceManaged in Test) (_ / "scrooge")}.value,
    (scroogeIsDirty in Compile) := {(scroogeIsDirty in Compile) map { (_) => true }}.value,
    publishArtifact in (Compile, packageDoc) := false,
    sources in (Compile,doc) := Seq.empty
  )
lazy val server = baseproject("web")
  .dependsOn(protocols)
...
lazy val user = baseproject("user")
  ...
  .settings(
    commonDocker,
    dockerImageNames("user")
  )
lazy val order = baseproject("orders")
  .dependsOn(protocols)
  .enablePlugins(sbtdocker.DockerPlugin, JavaAppPackaging)
  .settings(
    commonDocker,
    dockerImageNames("orders")
  )
lazy val preferences = baseproject("preferences")
  .dependsOn(protocols)
  .enablePlugins(sbtdocker.DockerPlugin, JavaAppPackaging)
  .settings(
    commonDocker,
    dockerImageNames("preferences")
  )

lazy val all = (project in file("."))
  .aggregate(server, user, order, preferences)
